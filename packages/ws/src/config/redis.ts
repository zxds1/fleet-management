// packages/ws/src/config/redis.ts
// Redis wiring for the gateway: the ConfigClient cache (00 §6) and the concurrent-session store
// (A1.6 / 02 §6 — `user:{userId}:sessions` sorted set, member = session_id, score = expires_at).
// The key namespace is intentionally identical to @fleet/api so the 10-session cap is enforced
// across BOTH processes (R-109). A Redis outage degrades to the in-memory stand-in (tests / dev).
...
export interface SessionStore {
  ...
  /** False when Redis is unavailable, so callers can fall back to the DB path (R-109). */
