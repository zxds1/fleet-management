...
@Composable
fun ResourceLibraryScreen(repository: FleetRepository, nav: NavController, locale: String) {
    val lessons by repository.trainingLessons.collectAsState()
    Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp)) {
        BackButton({ nav.popBackStack() }, locale)
        Spacer(Modifier.height(8.dp))
        ScreenTitle(t(locale, "resources.title"))
        Spacer(Modifier.height(16.dp))
        if (lessons.isEmpty()) {
            EmptyState(Icons.Filled.MenuBook, t(locale, "resources.title"), "No resources available.")
            return
        }
        lessons.forEach { l ->
            SectionCard(modifier = Modifier.fillMaxWidth().padding(bottom = 12.dp).clickable { nav.navigate("lesson_detail/${l.id}") }) {
                ...
                Text("${l.category} • ${l.durationMinutes} min reference", style = MaterialTheme.typography.bodyMedium, color = BentoTextSecondary)
            }
        }
    }
}
